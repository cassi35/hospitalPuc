...
class PrescricaoUpdateUseCase(PrescricaoUpdateInterface):
    ...
    def update(self, prescricao_id: int, prescricao: Prescricao) -> Dict:
        ...
        self.__update_prescricao(prescricao_id, prescricao)
    ...
    def __validate_informations(self, prescricao: Prescricao) -> None:
        # Validar paciente_id se fornecido
        if prescricao.paciente_id is not None:
            ...
            paciente = self.paciente_repository.select_paciente(prescricao.paciente_id)
            if not paciente:
                raise HttpBadRequestError("Paciente não encontrado")

        # Validar medico_id se fornecido
        if prescricao.medico_id is not None:
        ...
        if prescricao.medicamento_id is not None:
        ...
        # Validar data_prescricao se fornecida
        if prescricao.data_prescricao is not None:
            try:
        ...
        # Validar dosagem se fornecida
        if prescricao.dosagem is not None:
        ...
        # Validar frequencia se fornecida
        if prescricao.frequencia is not None:
