...
class MedicoUpdateUseCase(MedicoUpdateInterface):
    ...
    def update(self, medico_id: int, medico: Medico) -> Dict:
        ...
        self.__medico_exists(medico_id=medico_id)
        self.__update_medico(
                medico_id=medico_id,
                status=medico.status,
                nome=medico.nome,
                cpf=medico.cpf,
    ...
    def __validate_informations(self,nome:str,cpf:str,especialidade_id:int,telefone:str,email:str,status:str)-> None:
        ...
        return None
    def __medico_exists(self,medico_id:int)-> None:
        medico = self.medico_repository.findById(id=medico_id)
        if not medico:
            raise HttpNotFoundError("Medico não encontrado")
    def __update_medico(self,medico_id:int,nome:str,cpf:str,especialidade_id:int,telefone:str,email:str,status:str)-> None:
        self.medico_repository.update(
            cpf=cpf,
            email=email,
